<?php
/**
 * gridsby admin functions
 *
 * @package gridsby 
 */

// admin area styling
function gridsby_custom_colors() {
...
add_action('admin_head', 'gridsby_custom_colors'); 
...
add_action( 'tgmpa_register', 'gridsby_recommend_plugin' );
function gridsby_recommend_plugin() {
    ...
    $plugins = array(
		...
		array(
			'name'		=> 'RICG Responsive Images', // https://wordpress.org/plugins/ricg-responsive-images/ 
...
function gridsby_FontAwesome_icons() {
    wp_enqueue_style( 'gridsby-font-awesome-admin', get_template_directory_uri() . '/fonts/font-awesome.css' );
}

add_action('admin_head', 'gridsby_FontAwesome_icons'); 
add_action('wp_head', 'gridsby_FontAwesome_icons');
