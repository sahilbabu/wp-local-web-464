...
/*-----------------------------------------------------------------------------------*/
	...
	function jellywp_subscribe_box(){
		$widget_ops = array( 'classname' => 'jellywp_subscribe_box', 'description' => esc_attr__( 'Email subscribe box' , 'jelly_text_main') );
		parent::__construct('jellywp_subscribe_box', esc_attr__('jellywp: Email subscribe box', 'jelly_text_main'), $widget_ops);
	}
...
<form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo esc_attr($feed_url); ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
		     <input type="text" class="text" name="email" value="<?php _e('Your Email', 'jelly_text_main'); ?>" onfocus="if(this.value=='<?php _e('Your Email', 'jelly_text_main'); ?>')this.value='';" onblur="if(this.value=='')this.value='<?php _e('Your Email', 'jelly_text_main'); ?>';"/>
		     <input type="hidden" name="loc" value="en_US"/>
			<input type="hidden" value="<?php echo esc_attr($feed_url); ?>" name="uri"/>
		     <input type="submit"  class="buttons" value="<?php esc_attr_e('Subscribe', 'jelly_text_main');?>" />
                     </form>
...
/*-----------------------------------------------------------------------------------*/
	...
	function form($instance){
		?>
		<?php
			$defaults = array( 'title' => __( 'Email Subscribe' , 'jelly_text_main') , 'feed_description' => 'Mauris mattis auctor cursus. Phasellus tellus tellus, imperdiet ut imperdiet eu, iaculis a sem.', 'feed_url' => '' );
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'feed_url' )); ?>"><?php esc_attr_e('feedburner name: (your name without http://feeds.feedburner.com/)', 'jelly_text_main'); ?></label>
