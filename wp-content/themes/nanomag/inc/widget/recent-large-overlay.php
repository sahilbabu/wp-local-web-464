<?php
add_action('widgets_init', 'recent_large_overlay_widgets');

function recent_large_overlay_widgets() {
    register_widget('recent_large_overlay_widgets');
}

class recent_large_overlay_widgets extends WP_Widget {
...
/*-----------------------------------------------------------------------------------*/
    ...
    function __construct() {
        $widget_ops = array(
            'classname' => 'main_post_large_overlay_style clearfix',
            'description' => esc_attr__('Display post with text overlay on menu or widget.', 'jelly_text_main')
        );
        parent::__construct('recent-post-large-overlay-style', esc_attr__('jellywp: Recent post large text overlay', 'jelly_text_main'), $widget_ops);
    }
        ...
        while ($jellywp_widget->have_posts()) {
			...
			$post_id = get_the_ID();
			    ?>   
                 ...
                 <div class="large_main_post_widget feature_post_images_w <?php if(!of_get_option('disable_css_animation')==1){echo esc_attr("appear_animation");}?>">
