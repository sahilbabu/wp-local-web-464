<?php
add_action('widgets_init', 'recent1_whitebg_widgets');

function recent1_whitebg_widgets() {
    register_widget('recent1_post_whitebg_widgets');
}

class recent1_post_whitebg_widgets extends WP_Widget {
...
/*-----------------------------------------------------------------------------------*/
    ...
    function __construct() {
        $widget_ops = array(
            'classname' => 'main_post_style_white clearfix',
            'description' => esc_attr__('Display a list of recent post with no background.', 'jelly_text_main')
        );
        parent::__construct('home-main-post-white-style', esc_attr__('jellywp: main post images white', 'jelly_text_main'), $widget_ops);
    }
        ...
        while ($jellywp_widget->have_posts()) {
            ...
            if ($i == 1) {
                ?>   
                    ...
                    <div class="two-content-wrapper feature_post_images_w <?php if(!of_get_option('disable_css_animation')==1){echo esc_attr("appear_animation");}?>">
...
<p class="mainpost_white_widget_desc"><?php echo jelly_post_list_excerpt(get_the_excerpt('')); ?> </p>
</div> 
    </div>
