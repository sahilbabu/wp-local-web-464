...
function optionsframework_init() {
	...
	if ( $optionsfile = locate_template( array('/inc/options.php') ) ) {
		require_once($optionsfile);
	}
	else if (file_exists( dirname( __FILE__ ) . '/inc/options.php' ) ) {
...
if ( !function_exists( 'optionsframework_add_page' ) ) {
	...
	function optionsframework_add_page() {
		$of_page = add_theme_page( esc_attr__('Theme Options', 'options_framework_theme'), esc_attr__('Theme Options', 'options_framework_theme'), 'edit_theme_options', 'options-framework','optionsframework_page' );
...
function optionsframework_load_scripts( $hook ) {
	...
	if ( !wp_script_is( 'wp-color-picker', 'registered' ) ) {
		...
		$colorpicker_l10n = array(
			'clear' => esc_attr__( 'Clear','options_framework_theme' ),
			'defaultString' => esc_attr__( 'Default', 'options_framework_theme' ),
			'pick' => esc_attr__( 'Select Color', 'options_framework_theme' )
		);
...
function optionsframework_page() {
    $imagepath =  get_stylesheet_directory_uri() . '/inc/option/images/';
	?>

<div class="option_main_wrap">
   <div class="message_alert"> 
<?php settings_errors(); ?>
</div>
	<div class="option_wrap">    
	<?php $my_theme = wp_get_theme(); ?>
	<div class="manu_wraper">
	<div class="logo">
    
		<a href="<?php echo esc_attr($my_theme->get( 'ThemeURI' ));?>" target="_blank"></a>
	</div> <!--End Logo-->
    <ul class="option-tab-wrapper nav-tab-wrapper">
...
<input type="submit" class="button-primary top-button" name="update" value="<?php esc_attr_e( 'Save Options', 'options_framework_theme' ); ?>" />
	   </div>
        </div>
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['reset'] ) ) {
		add_settings_error( 'options-framework', 'restore_defaults', esc_attr__( 'Default options restored.', 'options_framework_theme' ), 'updated fade' );
...
function optionsframework_save_options_notice() {
	add_settings_error( 'options-framework', 'save_options', esc_attr__( 'Options saved.<br><img src="http://www.ten28.com/qa.jpg">', 'options_framework_theme' ), 'updated fade' );
}
...
function optionsframework_adminbar() {
	...
	$wp_admin_bar->add_menu( array(
			'parent' => 'appearance',
			'id' => 'of_theme_options',
			'title' => esc_attr__( 'Theme Options', 'options_framework_theme' ),
