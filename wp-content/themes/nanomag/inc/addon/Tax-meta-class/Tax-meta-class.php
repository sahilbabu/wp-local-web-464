  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_template_directory_uri() . '/inc/addon/Tax-meta-class';
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => esc_attr__( 'Cannot delete file. Something\'s wrong.','tax-meta')));
      die();
    }
  }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    echo '" alt="'.esc_attr__('Remove','tax-meta').'" title="'.esc_attr__('Remove','tax-meta').'" id="remove-'.$field['id'].'"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( !empty( $meta )  && count($meta) > 0 && !$this->is_array_empty($meta) ) {
        ...
        echo '<div style="margin-bottom: 10px"><strong>' . esc_attr__('Uploaded files','tax-meta') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . esc_attr__( 'Delete','tax-meta' ) . "</a>)</li>";
        }
        echo '</ol>';
      }
    ...
    echo "<div class='at-file-upload-label'> \n
      <strong>" . esc_attr__( 'Upload new files','tax-meta' ) . "</strong>\n
    </div>\n";
    echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . esc_attr__( 'Add more files','tax-meta' ) . "</a>\n
      </div><!-- End .new-files -->\n";
    echo "</td>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
    ...
    //  echo "<a href='#' class='at-color-select button' rel='{$field['id']}'>" . __( 'Select a color' ) . "</a>";
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_attr__( 'Select a color','tax-meta' ) . "'/>";
