<?php
class home_post_list_medium extends AQ_Block {
	
	//set and create block
	function __construct() {
		$block_options = array(
		    'name' => esc_attr__('Home list post', 'jelly_text_main'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('home_post_list_medium', $block_options);
	}
	...
	function form($instance) {
        ...
        $css_class_builder = isset($instance['css_class_builder']) ? esc_attr($instance['css_class_builder']) : 'color-6';
		?>
    ...
    <div class="feature-post-list list-post-builder no-load-more-list-home loop-post-content <?php if(!of_get_option('disable_css_animation')==1){echo esc_attr("appear_animation");}?>">    
...
<a class="more_button_post" href="<?php the_permalink(); ?>"><?php _e('Read More', 'jelly_text_main'); ?></a>
