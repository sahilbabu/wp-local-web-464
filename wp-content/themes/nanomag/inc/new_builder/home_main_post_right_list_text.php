<?php
class home_post_right_list_text extends AQ_Block {
	
	//set and create block
	function __construct() {
		$block_options = array(
	    'name' => esc_attr__('Home post with right post text', 'jelly_text_main'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('home_post_right_list_text', $block_options);
	}
	
	
	//create form
	function form($instance) {
        $titles = isset($instance['titles']) ? esc_attr($instance['titles']) : 'Home with right post text';
    ...
    $css_class_builder = isset($instance['css_class_builder']) ? esc_attr($instance['css_class_builder']) : 'color-4';
        ?>
   ...
   <p>
	...
	function block($instance) {
        ...
        if (!$number_show = absint($instance['number_show'])){$number_show = 6;}			
        ...
        $i = 0;?>
        <div class="widget main_post_style main_right_post_style_list home_post_right_list_text clearfix <?php echo esc_attr($instance['css_class_builder']);?>">
