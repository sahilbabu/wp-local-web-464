...
class BWP_GXS_MODULE_POST extends BWP_GXS_MODULE {
    ...
    protected function generate_data() {
        ...
        $exclude_post_sql = sizeof($excluded_posts) > 0 ? ' AND p.ID NOT IN (' . implode(',', $excluded_posts) . ') ' : '';
        ...
        for ($i = 0; $i < sizeof($latest_posts); $i++) {
            ...
            $this->data[] = $data;
            $this->generate_data_for_post_content($post);
        }
    ...
    function generate_data_for_post_content($post) {
        $data = array();
        $post_id = $post->ID;
